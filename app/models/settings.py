import enum
from sqlalchemy import Column, Integer, DateTime, func, String, JSON, Enum as AlchemyEnum
...
class AdminLog(Base):
	...
	entity = Column(String, nullable=False)  # object: "settings", "subscription_plan", ...
	entity_id = Column(String, nullable=True)  # object_id: ID плану (якщо є)
	changes = Column(JSON, nullable=False)  # {"field": "base_rate", "old": 100, "new": 150}
	created_at = Column(DateTime(timezone=True), server_default=func.now())
